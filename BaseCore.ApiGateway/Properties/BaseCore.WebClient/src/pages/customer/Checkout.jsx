...
import { orderApi, cartApi, productApi } from '../../services/api';
...
const Checkout = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Xóa cart trên DB sau khi đặt hàng thành công
            // (Nếu chỉ mua 1 phần thì chỉ xóa các item đã mua)
            const selectedJson = sessionStorage.getItem('checkout_selected_product_ids');
            if (selectedJson) {
                sessionStorage.removeItem('checkout_selected_product_ids');
                // Xóa từng item đã đặt hàng
                // Cart items are removed by the checkout API.
            } else {
                sessionStorage.removeItem('checkout_selected_product_ids');
            }
    ...
    return (
        <div style={{ maxWidth: '800px', margin: '40px auto', padding: '0 20px' }}>
            ...
            <h2 style={{ marginBottom: '20px', fontWeight: '900', color: '#0f172a' }}>Xác nhận thanh toán</h2>
