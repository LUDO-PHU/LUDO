        ...
        try {
            if (mode === 'register') {
                ...
                if (regResult && regResult.success === false) {
                    setError(regResult.message || 'Đăng ký thất bại, tên đăng nhập có thể đã tồn tại.');
                    setLoading(false);
                    return;
                }
            }
            ...
            setError(err.message || err.response?.data?.message || 'Không thể xử lý yêu cầu. Kiểm tra backend API rồi thử lại.');
        } finally {
            setLoading(false);
        }
    };
        ...
        <main className="auth-screen">
            <section className="auth-panel">
                <div className="auth-panel__visual">
                    <Link to="/customer/home" className="brand-mark auth-brand">
                        <span className="brand-mark__icon"><i className="fa fa-mobile-alt"></i></span>
                        <span className="brand-mark__text">PhoneStore</span>
                    </Link>
                    <div>
                        <span className="eyebrow">Kênh mua hàng</span>
                        <h1>Linh kiện điện thoại rõ giá, dễ chọn.</h1>
                        <p>Đăng nhập để theo dõi đơn hàng, lưu giỏ hàng và nhận thông báo khuyến mãi.</p>
                    </div>
                    <img src="/images/products/kinhcuongluc.jpg" alt="PhoneStore product" />
                </div>
                        ...
                        {mode === 'register' && (
                            <div className="form-group">
                                ...
                                <div className="account-type-grid">
                                    ...
                                    <button type="button" className={form.userType === 1 ? 'is-active' : ''} onClick={() => updateForm('userType', 1)}>
